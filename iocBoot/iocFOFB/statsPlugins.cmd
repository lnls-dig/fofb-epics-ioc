...
NDStatsConfigure("STATSPM_CH0", $(QSIZE), 0, "$(PORT)", 1, 0, 0)
dbLoadRecords("$(AREA_DETECTOR_COMPAT)/db/NDStats.template",     "P=${P},R=${R}PM_CH0_STATS, PORT=STATSPM_CH0,ADDR=0,TIMEOUT=1,NDARRAY_PORT=$(PORT),NDARRAY_ADDR=12,HIST_SIZE=256,XSIZE=$(RING_SIZE),YSIZE=0,NCHANS=$(NCHANS)")
NDTimeSeriesConfigure("STATSPM_CH0_TS", $(QSIZE), 0, "STATSPM_CH0", 1, 27)
dbLoadRecords("$(AREA_DETECTOR_COMPAT)/db/NDTimeSeries.template",  "P=${P},R=${R}PM_CH0_TS_STATS, PORT=STATSPM_CH0_TS,ADDR=0,TIMEOUT=1,NDARRAY_PORT=STATSPM_CH0,NDARRAY_ADDR=12,NCHANS=$(NCHANS),ENABLED=0")
